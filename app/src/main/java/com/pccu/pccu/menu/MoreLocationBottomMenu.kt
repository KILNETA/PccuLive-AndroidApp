...
class MoreLocationBottomMenu(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for( j in CwbSource.CWB_locationArea.indices) {
            //區域分隔線 控件
            val textArea = TextView(this.requireContext())
            textArea.layoutParams =
            ...
            for (i in CwbSource.CWB_locations[j].indices) {
                //縣市選項 控件
                val textLocation = TextView(this.requireContext())
                textLocation.layoutParams =
                    ViewGroup.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.WRAP_CONTENT
                    )
