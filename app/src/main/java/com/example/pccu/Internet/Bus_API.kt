...
object Bus_API{
    ...
    val base_Url = "https://tdx.transportdata.tw/"
     ...
     * @since Alpha_5.0
     */
    fun Get_token(): TDX_token? {
        val Url = "https://tdx.transportdata.tw/"
        return HttpRetrofit.create_Json(ApiServce::class.java,Url).getTDX_token(
    ...
    /**
     * 公車API查詢
     * @param token [TDX_token] API_token許可證
     * @param activity [String] API操作目標
     * @param request [BUS_ApiRequest] 查詢用參數
     * @return Json文檔 : [ResponseBody]
     *
     * @author KILNETA
     * @since Alpha_5.0
     */
    fun Get(token:TDX_token, activity:String , request :BUS_ApiRequest): ResponseBody {
        //回傳 路線站點表
        return HttpRetrofit.create_Json(
            ApiServce::class.java,                  //Api資料節點接口
            base_Url                                //根網域
        ).getTDX_CR(
...
data class busLocation(
    val ID: String,         //用於存取API服務的token，格式為JWT
    val LocationName: NameType,              //token的有效期限
) : Serializable
 ...
 * @param SubRouteID            Array<[String]> 地區既用中之附屬路線代碼
...
data class BusSubRoute(
    ...
    val OperatorIDs: Array<String>, //營運業者代碼
    val SubRouteName: NameType,     //附屬路線名稱
    val Headsign: String = "",           //車頭描述
 ...
 * @param SubRoutes                     Array<[BusSubRoute]> 附屬路線資料
...
data class BusRoute(
    ...
    val Operators: Array<RouteOperator>,    //營運業者
    val AuthorityID: String,                //業管機關代碼
    val ProviderID: String,                 //資料提供平台代碼
    val SubRoutes: Array<BusSubRoute>,      //附屬路線資料
                                            //  (如果原始資料並無提供附屬路線ID，而本平台基於跨來源資料之一致性，會以SubRouteID=RouteID產製一份相對應的附屬路線資料
                                            //  (若有去返程，則會有兩筆))
...
data class BufferZones(
    val BufferZones : Array<BufferZone>,//緩衝區資訊
    val Fares : Array<Fare>,            //每段收費資訊
) : Serializable
...
data class BusFare(
    ...
    val FareClass : Int,    //費率等級 {
                                    ...
                                    // 8:團體 9:軍警 10:由各運業者自行定義的半票 }
    val Price : Int,        //計費價格(新台幣) (其中-1表示不提供售票服務)
    val DiscountPeriods : Array<BusDiscountPeriods>,
                            //優惠時段
) : Serializable
...
data class Bus_Data_Fare(
    ...
    val IsForAllSubRoutes : Int,            //該收費方式是否應用到所有附屬路線 {
                                                    // 0:否 1:是 }
    val SectionFares : Array<BufferZones>,  //段次計費
    val StageFares : Array<BusStageFare>,   //計費站區間計費
    val ODFares : Array<BusODFare>,         //起迄站間計費
...
data class SaveBusList(
    var ListName : String,
    val canLost : Boolean,
    val SaveStationList: ArrayList<SaveStation> = arrayListOf()
) : Serializable