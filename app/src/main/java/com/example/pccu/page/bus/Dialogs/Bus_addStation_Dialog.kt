...
class Bus_addStation_Dialog : DialogFragment(R.layout.bus_dialog) {
    ...
    var saveStation : CollectStation? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        saveStation = getArguments()!!.getSerializable("saveStation") as CollectStation  //拆包裹
                ...
                GlobalScope.launch ( Dispatchers.Main ) {
                    if(it) {
                        if(CollectList!![index].SaveStationList.any { it.StationUID == saveStation!!.StationUID && it.Direction == saveStation!!.Direction }){
                            val toast: Toast =
                                Toast.makeText(context!!, "${CollectList!![index].GroupName} 已存在此站牌", Toast.LENGTH_SHORT)
                            ...
                            val toast: Toast =
                                Toast.makeText(context!!, "成功添加至 ${CollectList!![index].GroupName}", Toast.LENGTH_SHORT)
                            toast.setGravity(Gravity.CENTER, 0, 0)
                            toast.show()
                        }
                    }
    ...
    inner class Adapter : RecyclerView.Adapter<MyViewHolder>() {
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            holder.itemView.GroupName.text = Group.GroupName
